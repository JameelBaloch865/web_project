import "./Frame.css";

const Frame = () => {
  return (
    <div className="frame">
      <div className="frame-child" />
      <div className="frame-item" />
      <div className="frame-inner" />
      <div className="frame-parent">
        <div className="womans-fashion-parent">
          <div className="womans-fashion">Woman’s Fashion</div>
          <img className="dropdown-icon" alt="" src="/dropdown@2x.png" />
        </div>
        <div className="mens-fashion-parent">
          <div className="womans-fashion">Men’s Fashion</div>
          <img className="dropdown-icon" alt="" src="/dropdown@2x.png" />
        </div>
        <div className="womans-fashion">Electronics</div>
        <div className="womans-fashion">{`Home & Lifestyle`}</div>
        <div className="womans-fashion">Medicine</div>
        <div className="womans-fashion">{`Sports & Outdoor`}</div>
        <div className="womans-fashion">{`Baby’s & Toys`}</div>
        <div className="womans-fashion">{`Groceries & Pets`}</div>
        <div className="womans-fashion">{`Health & Beauty`}</div>
      </div>
      <div className="view-all-products-wrapper">
        <div className="view-all-products">View All Products</div>
      </div>
      <div className="ellipse-parent">
        <div className="ellipse-div" />
        <div className="enhance-your-music">Enhance Your Music Experience</div>
        <div className="flash-sell-timer-parent">
          <div className="flash-sell-timer">
            <div className="flash-sell-timer-child" />
            <div className="parent">
              <div className="div">23</div>
              <div className="hours">Hours</div>
            </div>
          </div>
          <div className="flash-sell-timer">
            <div className="flash-sell-timer-child" />
            <div className="group">
              <div className="div">05</div>
              <div className="days">Days</div>
            </div>
          </div>
          <div className="flash-sell-timer">
            <div className="flash-sell-timer-child" />
            <div className="container">
              <div className="div">59</div>
              <div className="minutes">Minutes</div>
            </div>
          </div>
          <div className="flash-sell-timer">
            <div className="flash-sell-timer-child" />
            <div className="frame-div">
              <div className="div">35</div>
              <div className="seconds">Seconds</div>
            </div>
          </div>
        </div>
        <div className="button">
          <div className="view-all-products">Buy Now!</div>
        </div>
        <div className="categories">Categories</div>
        <div className="jbl-boombox-2-hero-020-x1-1-wrapper">
          <img
            className="jbl-boombox-2-hero-020-x1-1"
            alt=""
            src="/jbl-boombox-2-hero-020-x1-1-1@2x.png"
          />
        </div>
      </div>
      <div className="frame-group">
        <div className="frame-container">
          <div className="frame-parent1">
            <div className="frame-parent2">
              <div className="instance-parent">
                <div className="rectangle-wrapper">
                  <div className="instance-child" />
                </div>
                <div className="div">Today’s</div>
              </div>
              <div className="flash-sales">Flash Sales</div>
            </div>
            <div className="group-div">
              <div className="days-parent">
                <div className="days1">Days</div>
                <b className="b">03</b>
              </div>
              <div className="hours-parent">
                <div className="hours1">Hours</div>
                <b className="b1">23</b>
              </div>
              <div className="minutes-parent">
                <div className="minutes1">Minutes</div>
                <b className="b2">19</b>
              </div>
              <div className="seconds-parent">
                <div className="seconds1">Seconds</div>
                <b className="b1">56</b>
              </div>
              <div className="semiclone">
                <div className="semiclone-child" />
                <div className="semiclone-child" />
              </div>
              <div className="semiclone1">
                <div className="semiclone-child" />
                <div className="semiclone-child" />
              </div>
              <div className="semiclone2">
                <div className="semiclone-child" />
                <div className="semiclone-child" />
              </div>
            </div>
          </div>
          <div className="fill-with-left-arrow-parent">
            <img
              className="fill-with-left-arrow"
              alt=""
              src="/fill-with-left-arrow@2x.png"
            />
            <img
              className="fill-with-left-arrow"
              alt=""
              src="/fill-with-right-arrow@2x.png"
            />
          </div>
        </div>
        <div className="cart-with-flat-discount-parent">
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="discount-percent">
                <div className="new">-40%</div>
              </div>
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="g92-2-500x500-1-wrapper">
                <img
                  className="g92-2-500x500-1-icon"
                  alt=""
                  src="/g922500x500-1@2x.png"
                />
              </div>
            </div>
            <div className="havit-hv-g92-gamepad-parent">
              <div className="view-all-products">HAVIT HV-G92 Gamepad</div>
              <div className="parent1">
                <div className="view-all-products">$120</div>
                <div className="div6">$160</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                </div>
                <div className="div7">(88)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="discount-percent">
                <div className="new">-35%</div>
              </div>
              <div className="rectangle-div" />
              <div className="add-to-cart">Add To Cart</div>
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="ak-900-01-500x500-1-wrapper">
                <img
                  className="ak-900-01-500x500-1-icon"
                  alt=""
                  src="/ak90001500x500-1@2x.png"
                />
              </div>
            </div>
            <div className="havit-hv-g92-gamepad-parent">
              <div className="view-all-products">AK-900 Wired Keyboard</div>
              <div className="parent1">
                <div className="view-all-products">$960</div>
                <div className="div6">$1160</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon9" alt="" src="/vector@2x.png" />
                </div>
                <div className="div7">(75)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="discount-percent">
                <div className="new">-30%</div>
              </div>
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="g92-2-500x500-1-wrapper">
                <img
                  className="g27cq4-500x500-1-icon"
                  alt=""
                  src="/g27cq4500x500-1@2x.png"
                />
              </div>
            </div>
            <div className="havit-hv-g92-gamepad-parent">
              <div className="view-all-products">IPS LCD Gaming Monitor</div>
              <div className="parent1">
                <div className="view-all-products">$370</div>
                <div className="div6">$400</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                </div>
                <div className="div7">(99)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="discount-percent">
                <div className="new">-25%</div>
              </div>
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="g92-2-500x500-1-wrapper">
                <img
                  className="sam-moghadam-khamseh-kvmdstrgo-icon"
                  alt=""
                  src="/sammoghadamkhamsehkvmdstrgobmunsplash-1@2x.png"
                />
              </div>
            </div>
            <div className="havit-hv-g92-gamepad-parent">
              <div className="view-all-products">{`S-Series Comfort Chair `}</div>
              <div className="parent1">
                <div className="view-all-products">$375</div>
                <div className="div6">$400</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img
                    className="star-half-filled-icon"
                    alt=""
                    src="/starhalffilled@2x.png"
                  />
                </div>
                <div className="div7">(99)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="discount-percent">
                <div className="new">-25%</div>
              </div>
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="g92-2-500x500-1-wrapper">
                <img
                  className="sam-moghadam-khamseh-kvmdstrgo-icon"
                  alt=""
                  src="/sammoghadamkhamsehkvmdstrgobmunsplash-1@2x.png"
                />
              </div>
            </div>
            <div className="havit-hv-g92-gamepad-parent">
              <div className="view-all-products">{`S-Series Comfort Chair `}</div>
              <div className="parent1">
                <div className="view-all-products">$375</div>
                <div className="div6">$400</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img
                    className="star-half-filled-icon"
                    alt=""
                    src="/starhalffilled@2x.png"
                  />
                </div>
                <div className="div7">(99)</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="frame-parent3">
        <div className="frame-parent4">
          <div className="frame-parent5">
            <div className="instance-parent">
              <div className="rectangle-wrapper">
                <div className="instance-child" />
              </div>
              <div className="div">Categories</div>
            </div>
            <div className="flash-sales">Browse By Category</div>
          </div>
          <div className="fill-with-left-arrow-parent">
            <img
              className="fill-with-left-arrow"
              alt=""
              src="/fill-with-left-arrow@2x.png"
            />
            <img
              className="fill-with-left-arrow"
              alt=""
              src="/fill-with-right-arrow@2x.png"
            />
          </div>
        </div>
        <div className="category-phone-parent">
          <div className="category-phone">
            <div className="phones">Phones</div>
            <img
              className="category-cellphone-icon"
              alt=""
              src="/categorycellphone@2x.png"
            />
          </div>
          <div className="category-phone">
            <div className="computers">Computers</div>
            <img
              className="category-computer-icon"
              alt=""
              src="/categorycomputer@2x.png"
            />
          </div>
          <div className="category-phone">
            <div className="smartwatch">SmartWatch</div>
            <div className="category-smartwatch">
              <img className="vector-icon23" alt="" src="/vector@2x.png" />
              <img className="vector-icon24" alt="" src="/vector@2x.png" />
              <img className="vector-icon25" alt="" src="/vector@2x.png" />
              <img className="vector-icon26" alt="" src="/vector@2x.png" />
              <div className="line-parent">
                <div className="line-div" />
                <div className="frame-child1" />
                <div className="frame-child2" />
              </div>
            </div>
          </div>
          <div className="category-phone">
            <div className="camera">Camera</div>
            <img
              className="category-cellphone-icon"
              alt=""
              src="/categorycamera@2x.png"
            />
          </div>
          <div className="category-phone">
            <div className="smartwatch">HeadPhones</div>
            <img
              className="category-cellphone-icon"
              alt=""
              src="/categoryheadphone@2x.png"
            />
          </div>
          <div className="category-phone">
            <div className="gaming">Gaming</div>
            <img
              className="category-cellphone-icon"
              alt=""
              src="/categorygamepad@2x.png"
            />
          </div>
        </div>
      </div>
      <div className="frame-parent6">
        <div className="frame-parent7">
          <div className="frame-parent5">
            <div className="instance-parent">
              <div className="rectangle-wrapper">
                <div className="instance-child" />
              </div>
              <div className="div">This Month</div>
            </div>
            <div className="flash-sales">Best Selling Products</div>
          </div>
          <div className="view-all-products-container">
            <div className="view-all-products">View All</div>
          </div>
        </div>
        <div className="category-phone-parent">
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="g92-2-500x500-1-wrapper">
                <img
                  className="zah9d-5626-002-100-0000-light-icon"
                  alt=""
                  src="/672462-zah9d-5626-002-100-0000-lightthenorthfacexguccicoat-1@2x.png"
                />
              </div>
            </div>
            <div className="the-north-coat-parent">
              <div className="view-all-products">The north coat</div>
              <div className="parent1">
                <div className="view-all-products">$260</div>
                <div className="div6">$360</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                </div>
                <div className="div7">(65)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="g92-2-500x500-1-wrapper">
                <img
                  className="c2st-8746-001-082-0000-light-g-icon"
                  alt=""
                  src="/547953-9c2st-8746-001-082-0000-lightguccisavoymediumdufflebag-1@2x.png"
                />
              </div>
            </div>
            <div className="the-north-coat-parent">
              <div className="view-all-products">Gucci duffle bag</div>
              <div className="parent1">
                <div className="view-all-products">$960</div>
                <div className="div6">$1160</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img
                    className="star-half-filled-icon"
                    alt=""
                    src="/starhalffilled@2x.png"
                  />
                </div>
                <div className="div7">(65)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="gammaxx-l240-argb-1-500x500-1-wrapper">
                <img
                  className="gammaxx-l240-argb-1-500x500-1-icon"
                  alt=""
                  src="/gammaxxl240argb1500x500-1@2x.png"
                />
              </div>
            </div>
            <div className="the-north-coat-parent">
              <div className="view-all-products">RGB liquid CPU Cooler</div>
              <div className="parent1">
                <div className="view-all-products">$160</div>
                <div className="div6">$170</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img
                    className="star-half-filled-icon"
                    alt=""
                    src="/starhalffilled@2x.png"
                  />
                </div>
                <div className="div7">(65)</div>
              </div>
            </div>
          </div>
          <div className="cart-with-flat-discount">
            <div className="discount-percent-parent">
              <div className="fill-heart-parent">
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-heart@2x.png"
                />
                <img
                  className="fill-heart-icon"
                  alt=""
                  src="/fill-eye@2x.png"
                />
              </div>
              <div className="gammaxx-l240-argb-1-500x500-1-wrapper">
                <img
                  className="sam-moghadam-khamseh-l-7mqshl-icon"
                  alt=""
                  src="/sammoghadamkhamsehl-7mqshl-auunsplash-1@2x.png"
                />
              </div>
            </div>
            <div className="the-north-coat-parent">
              <div className="view-all-products">Small BookSelf</div>
              <div className="wrapper">
                <div className="view-all-products">$360</div>
              </div>
              <div className="five-star-parent">
                <div className="five-star">
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                  <img className="vector-icon" alt="" src="/vector@2x.png" />
                </div>
                <div className="div7">(65)</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="frame-parent9">
        <div className="frame-parent10">
          <div className="frame-parent11">
            <div className="frame-parent5">
              <div className="instance-parent">
                <div className="rectangle-wrapper">
                  <div className="instance-child" />
                </div>
                <div className="div">Our Products</div>
              </div>
              <div className="flash-sales">Explore Our Products</div>
            </div>
            <div className="fill-with-left-arrow-parent">
              <img
                className="fill-with-left-arrow"
                alt=""
                src="/fill-with-left-arrow@2x.png"
              />
              <img
                className="fill-with-left-arrow"
                alt=""
                src="/fill-with-right-arrow@2x.png"
              />
            </div>
          </div>
          <div className="frame-parent13">
            <div className="cart-group">
              <div className="cart-with-flat-discount">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="g92-2-500x500-1-wrapper">
                    <img
                      className="rdoexxtrl-1-icon"
                      alt=""
                      src="/71rdoexxtrl-1@2x.png"
                    />
                  </div>
                </div>
                <div className="the-north-coat-parent">
                  <div className="view-all-products">Breed Dry Dog Food</div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$100</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon9"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon9"
                          alt=""
                          src="/vector@2x.png"
                        />
                      </div>
                      <div className="div7">(35)</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cart10">
                <div className="discount-percent-parent">
                  <div className="rectangle-div" />
                  <div className="add-to-cart1">Add To Cart</div>
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="eos-250d-03-500x500-1-wrapper">
                    <img
                      className="eos-250d-03-500x500-1-icon"
                      alt=""
                      src="/eos250d03500x500-1@2x.png"
                    />
                  </div>
                </div>
                <div className="canon-eos-dslr-camera-parent">
                  <div className="view-all-products">CANON EOS DSLR Camera</div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$360</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon9"
                          alt=""
                          src="/vector@2x.png"
                        />
                      </div>
                      <div className="div7">(95)</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cart-with-flat-discount">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="g92-2-500x500-1-wrapper">
                    <img
                      className="ideapad-gaming-3i-01-500x500-1-icon"
                      alt=""
                      src="/ideapadgaming3i01500x500-1@2x.png"
                    />
                  </div>
                </div>
                <div className="the-north-coat-parent">
                  <div className="view-all-products">
                    ASUS FHD Gaming Laptop
                  </div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$700</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                      </div>
                      <div className="div40">(325)</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="cart-with-flat-discount">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="g92-2-500x500-1-wrapper" />
                  <img
                    className="curology-j7pkvqrtusm-unsplash-icon"
                    alt=""
                    src="/curologyj7pkvqrtusmunsplash-1@2x.png"
                  />
                </div>
                <div className="the-north-coat-parent">
                  <div className="view-all-products">{`Curology Product Set `}</div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$500</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon9"
                          alt=""
                          src="/vector@2x.png"
                        />
                      </div>
                      <div className="div40">(145)</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="cart-with-flat-discount-parent">
              <div className="cart-with-flat-discount">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="new-button">
                    <div className="new">NEW</div>
                  </div>
                  <div className="g92-2-500x500-1-wrapper">
                    <img
                      className="new-mercedes-benz-gtr-licensed-icon"
                      alt=""
                      src="/newmercedesbenzgtrlicensedrideoncarkidselectrictoycar-1@2x.png"
                    />
                  </div>
                </div>
                <div className="havit-hv-g92-gamepad-parent">
                  <div className="view-all-products">Kids Electric Car</div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$960</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                      </div>
                      <div className="div7">(65)</div>
                    </div>
                  </div>
                  <div className="fill-with-left-arrow-parent">
                    <img
                      className="colour-chnage-child"
                      alt=""
                      src="/group-1000005935@2x.png"
                    />
                    <div className="colour-chnage-item" />
                  </div>
                </div>
              </div>
              <div className="cart17">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="g92-2-500x500-1-wrapper">
                    <img
                      className="copa-sense-1-icon"
                      alt=""
                      src="/copa-sense-1@2x.png"
                    />
                  </div>
                </div>
                <div className="the-north-coat-parent">
                  <div className="view-all-products">
                    Jr. Zoom Soccer Cleats
                  </div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$1160</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                      </div>
                      <div className="div7">(35)</div>
                    </div>
                  </div>
                  <div className="fill-with-left-arrow-parent">
                    <img
                      className="colour-chnage-child"
                      alt=""
                      src="/group-1000005935@2x.png"
                    />
                    <div className="colour-chnage-item" />
                  </div>
                </div>
              </div>
              <div className="cart-with-flat-discount">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="new-button">
                    <div className="new">NEW</div>
                  </div>
                  <div className="g92-2-500x500-1-wrapper">
                    <img
                      className="gp11-prd3-1-icon"
                      alt=""
                      src="/gp11-prd3-1@2x.png"
                    />
                  </div>
                </div>
                <div className="havit-hv-g92-gamepad-parent">
                  <div className="view-all-products">
                    GP11 Shooter USB Gamepad
                  </div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$660</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="star-half-filled-icon"
                          alt=""
                          src="/starhalffilled@2x.png"
                        />
                      </div>
                      <div className="div7">(55)</div>
                    </div>
                  </div>
                  <div className="fill-with-left-arrow-parent">
                    <img
                      className="colour-chnage-child"
                      alt=""
                      src="/group-1000005935@2x.png"
                    />
                    <div className="colour-chnage-item" />
                  </div>
                </div>
              </div>
              <div className="cart17">
                <div className="discount-percent-parent">
                  <div className="fill-heart-parent">
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-heart@2x.png"
                    />
                    <img
                      className="fill-heart-icon"
                      alt=""
                      src="/fill-eye@2x.png"
                    />
                  </div>
                  <div className="g92-2-500x500-1-wrapper">
                    <img
                      className="z8a1x-3475-001-100-0000-light-icon"
                      alt=""
                      src="/698717-z8a1x-3475-001-100-0000-lightreversiblequiltedsatinjacket-1@2x.png"
                    />
                  </div>
                </div>
                <div className="the-north-coat-parent">
                  <div className="view-all-products">Quilted Satin Jacket</div>
                  <div className="frame-parent14">
                    <div className="five-star">
                      <div className="view-all-products">$660</div>
                    </div>
                    <div className="three-star-parent">
                      <div className="five-star">
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="vector-icon"
                          alt=""
                          src="/vector@2x.png"
                        />
                        <img
                          className="star-half-filled-icon"
                          alt=""
                          src="/starhalffilled@2x.png"
                        />
                      </div>
                      <div className="div7">(55)</div>
                    </div>
                  </div>
                  <div className="fill-with-left-arrow-parent">
                    <img
                      className="colour-chnage-child"
                      alt=""
                      src="/group-1000005935@2x.png"
                    />
                    <div className="colour-chnage-item" />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="view-all-products-container">
          <div className="view-all-products">View All Products</div>
        </div>
      </div>
      <div className="frame-parent22">
        <div className="frame-parent5">
          <div className="instance-parent">
            <div className="rectangle-wrapper">
              <div className="instance-child" />
            </div>
            <div className="div">Featured</div>
          </div>
          <div className="flash-sales">New Arrival</div>
        </div>
        <div className="frame-parent24">
          <div className="ps5-slim-goedkope-playstation-parent">
            <img
              className="ps5-slim-goedkope-playstation-icon"
              alt=""
              src="/ps5slimgoedkopeplaystation-large-1@2x.png"
            />
            <div className="frame-parent25">
              <div className="cart-with-flat-discount">
                <div className="womens-collections">PlayStation 5</div>
                <div className="black-and-white">
                  Black and White version of the PS5 coming out on sale.
                </div>
              </div>
              <div className="shop-now-parent">
                <div className="view-all-products">Shop Now</div>
                <img
                  className="underline-icon"
                  alt=""
                  src="/underline@2x.png"
                />
              </div>
            </div>
          </div>
          <div className="frame-parent26">
            <div className="attractive-woman-wearing-hat-p-parent">
              <img
                className="attractive-woman-wearing-hat-p-icon"
                alt=""
                src="/attractivewomanwearinghatposingblackbackground-1@2x.png"
              />
              <div className="frame-parent27">
                <div className="cart-with-flat-discount">
                  <div className="womens-collections">Women’s Collections</div>
                  <div className="featured-woman-collections">
                    Featured woman collections that give you another vibe.
                  </div>
                </div>
                <div className="shop-now-parent">
                  <div className="view-all-products">Shop Now</div>
                  <img
                    className="underline-icon"
                    alt=""
                    src="/underline@2x.png"
                  />
                </div>
              </div>
            </div>
            <div className="frame-parent28">
              <div className="wrapper-ellipse-24-parent">
                <div className="wrapper-ellipse-24">
                  <img
                    className="wrapper-ellipse-24-child"
                    alt=""
                    src="/ellipse-24@2x.png"
                  />
                </div>
                <div className="amazon-echo-png-clipart-transp-wrapper">
                  <img
                    className="amazon-echo-png-clipart-transp-icon"
                    alt=""
                    src="/69694768-amazonechopngcliparttransparentamazonechopng-1@2x.png"
                  />
                </div>
                <div className="frame-parent29">
                  <div className="the-north-coat-parent">
                    <div className="womens-collections">Speakers</div>
                    <div className="amazon-wireless-speakers">
                      Amazon wireless speakers
                    </div>
                  </div>
                  <div className="shop-now-parent">
                    <div className="view-all-products">Shop Now</div>
                    <img
                      className="underline-icon"
                      alt=""
                      src="/underline@2x.png"
                    />
                  </div>
                </div>
              </div>
              <div className="wrapper-ellipse-24-parent">
                <div className="wrapper-ellipse-241">
                  <img
                    className="wrapper-ellipse-24-item"
                    alt=""
                    src="/ellipse-24@2x.png"
                  />
                </div>
                <div className="e82cd70aa6522dd785109a455904c-wrapper">
                  <img
                    className="e82cd70aa6522dd785109a455904c-icon"
                    alt=""
                    src="/652e82cd70aa6522dd785109a455904c@2x.png"
                  />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="frame-parent30">
        <div className="the-north-coat-parent">
          <div className="womens-collections">Perfume</div>
          <div className="amazon-wireless-speakers">GUCCI INTENSE OUD EDP</div>
        </div>
        <div className="shop-now-parent">
          <div className="view-all-products">Shop Now</div>
          <img className="underline-icon" alt="" src="/underline@2x.png" />
        </div>
      </div>
      <div className="instance-parent3">
        <div className="services-parent">
          <img className="services-icon" alt="" src="/services@2x.png" />
          <div className="free-and-fast-delivery-parent">
            <div className="free-and-fast">FREE AND FAST DELIVERY</div>
            <div className="free-delivery-for">
              Free delivery for all orders over $140
            </div>
          </div>
        </div>
        <div className="services-group">
          <img className="services-icon" alt="" src="/services@2x.png" />
          <div className="customer-service-parent">
            <div className="free-and-fast">24/7 CUSTOMER SERVICE</div>
            <div className="friendly-247-customer">
              Friendly 24/7 customer support
            </div>
          </div>
        </div>
        <div className="services-parent">
          <img className="services-icon" alt="" src="/services@2x.png" />
          <div className="free-and-fast-delivery-parent">
            <div className="free-and-fast">MONEY BACK GUARANTEE</div>
            <div className="friendly-247-customer">
              We reurn money within 30 days
            </div>
          </div>
        </div>
        <div className="services-parent1">
          <img className="services-icon" alt="" src="/services@2x.png" />
          <div className="free-and-fast-delivery-parent">
            <div className="free-and-fast">MONEY BACK GUARANTEE</div>
            <div className="friendly-247-customer">
              We reurn money within 30 days
            </div>
          </div>
        </div>
      </div>
      <div className="footer">
        <div className="underline-parent">
          <img className="underline-icon4" alt="" src="/underline@2x.png" />
          <div className="frame-wrapper">
            <div className="icon-copyright-parent">
              <img
                className="star-half-filled-icon"
                alt=""
                src="/iconcopyright@2x.png"
              />
              <div className="womans-fashion">
                Copyright Rimel 2022. All right reserved
              </div>
            </div>
          </div>
        </div>
        <div className="frame-parent31">
          <div className="frame-parent32">
            <div className="logo-parent">
              <div className="logo-parent">
                <div className="logo">
                  <b className="exclusive">Exclusive</b>
                </div>
                <div className="subscribe">Subscribe</div>
              </div>
              <div className="get-10-off">Get 10% off your first order</div>
            </div>
            <div className="send-mail">
              <div className="enter-your-email">Enter your email</div>
              <img className="icon-send" alt="" src="/iconsend@2x.png" />
            </div>
          </div>
          <div className="logo-parent">
            <div className="support">Support</div>
            <div className="bijoy-sarani-dhaka-dh-1515-parent">
              <div className="bijoy-sarani-dhaka">
                111 Bijoy sarani, Dhaka, DH 1515, Bangladesh.
              </div>
              <div className="womans-fashion">exclusive@gmail.com</div>
              <div className="womans-fashion">+88015-88888-9999</div>
            </div>
          </div>
          <div className="logo-parent">
            <div className="support">Account</div>
            <div className="bijoy-sarani-dhaka-dh-1515-parent">
              <div className="womans-fashion">My Account</div>
              <div className="womans-fashion">Login / Register</div>
              <div className="womans-fashion">Cart</div>
              <div className="womans-fashion">Wishlist</div>
              <div className="womans-fashion">Shop</div>
            </div>
          </div>
          <div className="logo-parent">
            <div className="support">Quick Link</div>
            <div className="bijoy-sarani-dhaka-dh-1515-parent">
              <div className="womans-fashion">Privacy Policy</div>
              <div className="womans-fashion">Terms Of Use</div>
              <div className="womans-fashion">FAQ</div>
              <div className="womans-fashion">Contact</div>
            </div>
          </div>
          <div className="logo-parent">
            <div className="logo-parent">
              <div className="support">Download App</div>
              <div className="save-3-with-app-new-user-only-parent">
                <div className="save-3-with">
                  Save $3 with App New User Only
                </div>
                <div className="qr-code-parent">
                  <div className="qr-code">
                    <div className="wrapper-qrcode-1">
                      <img
                        className="qrcode-1-icon"
                        alt=""
                        src="/qrcode-1@2x.png"
                      />
                    </div>
                  </div>
                  <div className="googleplay-parent">
                    <img
                      className="googleplay-icon"
                      alt=""
                      src="/googleplay@2x.png"
                    />
                    <div className="appstore">
                      <img
                        className="download-appstore-icon"
                        alt=""
                        src="/downloadappstore@2x.png"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="icon-facebook-parent">
              <img className="icon-send" alt="" src="/iconfacebook@2x.png" />
              <img className="icon-send" alt="" src="/icontwitter@2x.png" />
              <img className="icon-send" alt="" src="/iconinstagram@2x.png" />
              <img className="icon-send" alt="" src="/iconlinkedin@2x.png" />
            </div>
          </div>
        </div>
      </div>
      <img
        className="fill-with-up-arrow"
        alt=""
        src="/fill-with-up-arrow@2x.png"
      />
      <div className="top-header">
        <div className="frame-parent35">
          <div className="qr-code-parent">
            <div className="summer-sale-for">
              Summer Sale For All Swim Suits And Free Express Delivery - OFF
              50%!
            </div>
            <div className="shopnow">ShopNow</div>
          </div>
          <div className="english-parent">
            <div className="english">English</div>
            <img className="icon-send" alt="" src="/dropdown@2x.png" />
          </div>
        </div>
      </div>
      <div className="hero-endframe-cvklg0xk3w6e-la-parent">
        <img
          className="hero-endframe-cvklg0xk3w6e-la-icon"
          alt=""
          src="/hero-endframe--cvklg0xk3w6e-large-2@2x.png"
        />
        <div className="px-apple-gray-logo-1-parent">
          <img
            className="px-apple-gray-logo-1-icon"
            alt=""
            src="/1200pxapple-gray-logo-1@2x.png"
          />
          <div className="iphone-14-series">iPhone 14 Series</div>
        </div>
        <div className="up-to-10">Up to 10% off Voucher</div>
        <div className="frame-parent36">
          <div className="googleplay-parent">
            <div className="view-all-products">Shop Now</div>
            <div className="frame-child3" />
          </div>
          <img className="icon-send" alt="" src="/icons-arrowright@2x.png" />
        </div>
        <div className="frame-child4" />
        <div className="ellipse-group">
          <div className="frame-child5" />
          <div className="frame-child5" />
          <div className="frame-child7" />
          <div className="frame-child5" />
          <div className="frame-child5" />
        </div>
      </div>
      <div className="top-header1">
        <div className="frame-parent35">
          <div className="qr-code-parent">
            <div className="summer-sale-for">
              Summer Sale For All Swim Suits And Free Express Delivery - OFF
              50%!
            </div>
            <div className="shopnow">ShopNow</div>
          </div>
          <div className="english-parent">
            <div className="english">English</div>
            <img className="icon-send" alt="" src="/dropdown@2x.png" />
          </div>
        </div>
      </div>
      <div className="header">
        <div className="logo-group">
          <div className="logo">
            <b className="exclusive">Exclusive</b>
          </div>
          <div className="header-parent">
            <div className="header1">
              <div className="womans-fashion">Home</div>
              <img className="underline-icon5" alt="" src="/underline@2x.png" />
            </div>
            <div className="header2">
              <div className="about">Contact</div>
            </div>
            <div className="header3">
              <div className="about">About</div>
            </div>
            <div className="header4">
              <div className="about">Sign Up</div>
            </div>
          </div>
        </div>
        <div className="search-component-set-parent">
          <div className="search-component-set">
            <div className="what-are-you-looking-for-parent">
              <div className="what-are-you">What are you looking for?</div>
              <img className="icon-send" alt="" src="/component-2@2x.png" />
            </div>
          </div>
          <div className="wishlist-parent">
            <img className="wishlist-icon" alt="" src="/wishlist@2x.png" />
            <div className="cart1-with-buy">
              <img className="cart1-icon" alt="" src="/cart1@2x.png" />
              <div className="ellipse-container">
                <div className="group-child" />
                <div className="div52">2</div>
              </div>
            </div>
            <img className="user-icon" alt="" src="/user@2x.png" />
          </div>
        </div>
      </div>
      <img className="line-icon" alt="" src="/line-3@2x.png" />
      <div className="account-dropdoen">
        <div className="account-dropdown">
          <div className="frame-parent38">
            <div className="instance-parent">
              <img className="wishlist-icon" alt="" src="/user@2x.png" />
              <div className="english">Manage My Account</div>
            </div>
            <div className="instance-parent">
              <img className="icon-send" alt="" src="/iconmallbag@2x.png" />
              <div className="english">My Order</div>
            </div>
            <div className="instance-parent">
              <img className="icon-send" alt="" src="/iconcancel@2x.png" />
              <div className="english">My Cancellations</div>
            </div>
            <div className="instance-parent">
              <img className="icon-reviews" alt="" src="/iconreviews@2x.png" />
              <div className="english">My Reviews</div>
            </div>
            <div className="instance-parent">
              <img className="dropdown-icon" alt="" src="/iconlogout@2x.png" />
              <div className="english">Logout</div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Frame;
